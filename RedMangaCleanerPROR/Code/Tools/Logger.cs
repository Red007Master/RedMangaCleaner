...
internal class Logger
{
    ...
    public void Log(string iLogMassage, LogLevel iLogLevel, int iDepthLevel = 0)
    {
        ...
        if (iDepthLevel == 0)
        {
            result = $"[{DateTime.Now}] [{iLogLevel}]{new string(' ', SPC.Get(iLogLevel))} | [{iLogMassage}]";
        }
        else
        {
            result = $"[{DateTime.Now}] [{iLogLevel}]{new string(' ', SPC.Get(iLogLevel))} |{new string('-', iDepthLevel)}>[{iLogMassage}]";
        }
